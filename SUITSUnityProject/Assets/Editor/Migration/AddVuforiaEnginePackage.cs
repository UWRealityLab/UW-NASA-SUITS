...
{
    static readonly string sPackagesPath = System.IO.Path.Combine(Application.dataPath, "..", "Packages");
    static readonly string sManifestJsonPath = System.IO.Path.Combine(sPackagesPath, "manifest.json");
    const string VUFORIA_VERSION = "10.15.3";
    ...
    {
        var tarFilePaths = Directory.GetFiles(System.IO.Path.Combine(Directory.GetCurrentDirectory(), VUFORIA_TAR_FILE_DIR)).Where(f => f.EndsWith(".tgz"));
    ...
    {
        var dependencyDirectory = System.IO.Path.Combine(Directory.GetCurrentDirectory(), DEPENDENCIES_DIR);
    ...
    {
        var sourceFile = System.IO.Path.Combine(Directory.GetCurrentDirectory(), folder, fileName);
        var destFile = System.IO.Path.Combine(Directory.GetCurrentDirectory(), PACKAGES_RELATIVE_PATH, fileName);
